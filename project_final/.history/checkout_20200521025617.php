                            ...
                            <ul class="mobile-menu main-mobile-menu">
                                ...
                                <li class="menu-item-has-children">
                                    ...
                                    ?>
                                        <li class="menu-item-has-children">
                                            <a
                                                href="categorey.php?idProd=<?php echo $rowReadSec['id'] ?>"><?php echo $rowReadSec['category_name'] ?></a>
                                        </li>
                                        <?php } ?>
    ...
    if($CODE == $promo ){
        ...
        if(empty($prodctCard)){
            ...
            foreach ($prodctCard as $key => $singleProCard) {
              ...
              $discountPromr1= $total-($total*30/100) ;
              echo $discountPromr1;
											...
											if(isset($discountPromr1)){
                                              
                                            echo $GranTot= $discountPromr1+5;
                                            echo "<input type='hidden' value='$GranTot' name='Grand'>";
                                             $status="1";
                                            $querypromo="UPDATE customer_table SET
                                                                              	status_promo=$status
                                                                              WHERE id_customer = $id_customer";
                                           $resultProm=mysqli_query($conn,$querypromo);
                               ...
                               if($resOrder=mysqli_query($conn,$queryOrder)){
                                  ...
                                  $rowLasInsert = mysqli_fetch_assoc($res);
                                      ...
                                      if(empty($prodctCard)){
                                        ...
                                        foreach ($prodctCard as $key => $singleProCard) {
                                            $idProductIn=$singleProCard[0]['id_product'];
                                      $item="INSERT INTO `order_items`(order_id,product_id,quantity,list_price)
                                             VALUES                   ((SELECT id_order FROM order_table WHERE order_table.id_order = $order),(SELECT id_product FROM product WHERE product.id_product =$idProductIn),$quantity,$sub)";
                                             if($resItem =mysqli_query($conn,$item)){
                                                ...
                                                if($row=mysqli_fetch_assoc($resSelect)){
                                                 ...
                                                 $queryQuant="UPDATE product SET
                                                                                   product_qty=$post_quan
                                                                                    WHERE id_product = $img";
                                                 if($resultProduct=mysqli_query($conn,$queryQuant)){

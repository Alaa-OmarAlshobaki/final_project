...
class user{
	...
	function user_name($user_id){
		$sql = "SELECT * FROM customer_table WHERE id_customer=$user_id";
		$query= $this->pdo->prepare($sql);
		$query->bindParam(':user_id',$user_id,PDO::PARAM_STR);
		$query->execute();
		$result=$query->fetch(PDO::FETCH_ASSOC);
		return $result['full_name_customer'];
	}	
	...
	function login_check($email,$password){
		$sql = "SELECT * FROM customer_table WHERE 	email_customer=:email and password_customer=:password";
		$query= $this->pdo->prepare($sql);
	...
	function chat_history($user,$to_user){
		...
		$output = '<ul class="list-unstyled">';
		 foreach($results as $row)
		 {
		  ...
		  {
		   $user_name = '<b class="text-danger">'.$this->user_name($row['to_user_id']).'</b>';

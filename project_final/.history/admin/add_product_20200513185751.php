...
<body class="alt-menu sidebar-noneoverflow" data-spy="scroll" data-target="#navSection" data-offset="100">
    ...
    if (isset($_POST['btn_upload'])) {
       ...
       $name_category=str_replace(' ','',$rowRead['category_name']);
       $name_product= $rowRead['prod_name'];

       $s = ucfirst($name_category);
       $bar = ucwords(strtolower($s));
       $data = preg_replace('/\s+/', '', $bar);


       
       $sp = ucfirst($name_product);
       $barp = ucwords(strtolower($sp));
       $datap = preg_replace('/\s+/', '', $barp);
    ...
    foreach ($_FILES['image']['name'] as $key => $img) {
        ...
        $full = $target_dir.$img_title;

        if (move_uploaded_file($_FILES['image']["tmp_name"][$key], $full)) {
            $sqlRead = "INSERT INTO gallery (pro_id, image) VALUES ('$idProud','$full')";
            $rowSqlRead=mysqli_query($conn,$sqlRead);
        }
    }
      
    // foreach($_FILES['image'] as $key =>$tmp_name ) {

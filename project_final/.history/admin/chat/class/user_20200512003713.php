...
class user{
	...
	function login_check($email,$password){
		$sql = "SELECT * FROM admin WHERE 	email_customer=:email and password_customer=:password";
		$query= $this->pdo->prepare($sql);
	...
	function chat_history($user,$to_user){
		...
		$output = '<ul class="list-unstyled">';
		 foreach($results as $row)
		 {
		  ...
		  {
		   $user_name = '<b class="text-success h2">You</b>';
		  }
		  else
		  {
		   $user_name = '<b class="text-danger h3">'.$this->user_name($row['from_user_id']).'</b>';
		  }

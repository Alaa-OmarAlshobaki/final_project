                               ...
                               if($resOrder=mysqli_query($conn,$queryOrder)){
                                  ...
                                  $rowLasInsert = mysqli_fetch_assoc($res);
                                      ...
                                      if(empty($prodctCard)){
                                        ...
                                        foreach ($prodctCard as $key => $singleProCard) {
                                            $PID=$singleProCard[0]['id_product'];
        
                                      $item="INSERT INTO `order_items`(order_id,product_id,quantity,list_price)
                                             VALUES                   ((SELECT id_order FROM order_table WHERE order_table.id_order = $order),(SELECT id_product FROM product WHERE product.id_product =$PID),$quantity,$sub)";
                                             if($resItem =mysqli_query($conn,$item)){
                                                 $querSelect="SELECT product_qty FROM product WHERE id_product=$PID";

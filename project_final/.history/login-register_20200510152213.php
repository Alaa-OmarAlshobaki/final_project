...
if(isset($_POST['save'])){
	...
	$querySelect="SELECT email_customer FROM admin WHERE email_customer= '$email '";
	$resSelect=mysqli_query($conn,$querySelect);
	if($row=mysqli_fetch_assoc($resSelect)){
		echo "the email is used";
	}else{
		if($pass === $repass){
       $queryInsert="INSERT INTO customer_table (full_name_customer,phone_customer,email_customer,street_customer,city_customer,password_customer)
	                             VALUES('$name',$phone,'$email','$str','$add','$pass')";
